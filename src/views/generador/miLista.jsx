...
function List () {
    ...
    const destaca=(e)=>{
        ...
        let ur ="http://localhost:8080/marcarContenidoDestacado/"+ e;
        fetchFunction(ur);
    }
    ...
    useEffect(() => {
        ...
        let ur ="http://localhost:8080/listarContenidosGenerador/" + JSON.parse(sessionStorage.getItem("usuario")).email;
        fetchFunction(ur);
                ...
                {data.map((elem, index) => {
                    return (
                        <div style={imageSize} key={index} className="m-2">
                            ...
                            <botoon onClick={destaca(e.id)} ></botoon>
